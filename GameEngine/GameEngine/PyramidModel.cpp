...
void PyramidModel::privCreateVAO( const char * const modelFileName )
{
	...
	// Remove HACK
		...
		//Matrix Scale(SCALE, vScale);
		//Matrix Trans(TRANS, vPos);
		//Matrix Rot(ROT_ORIENT, vDof, vUp);
    ...
    // Load the combined data: ---------------------------------------------------------
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(Vert_xyzuvn) * this->numVerts, pyramidData, GL_STATIC_DRAW);
	  ...
	  // Load the index data: ---------------------------------------------------------
            ...
            /* Bind our 2nd VBO as being the active buffer and storing index ) */
	         ...
	         /* Copy the index data to our buffer */
		      glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(Tri_index) * this->numTris, pyramidTriList, GL_STATIC_DRAW);
